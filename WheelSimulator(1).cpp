...
WheelSimulator::WheelSimulator(float width, float rim_radius, 
    ...
    : width_(width),
      ...
      wheel_(rim_radius/100, width/100, 0.238f, wheel_filepath) // initializes wheel radius, width, and mass. TODO: load in from a wheel file, instead of hardcoded
{
...
void WheelSimulator::PrepareSimulation() {
    ...
    std::cout << "rim_radius_" << rim_radius_ <<std::endl;
...
void WheelSimulator::RunSimulationLoop() {
    ...
    float box_halfsize_x = rim_radius_/100 * 1.25f;
    float box_halfsize_y = width_/100 * 2.0f;
